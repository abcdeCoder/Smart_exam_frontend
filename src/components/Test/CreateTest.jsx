  ...
  const validate = () => {
    ...
    for (let property in questions) {
      ...
      if (question.length == 2 && question[1].split(" ").length < 3) {
        setErrorMsg((oldMsg) =>
          oldMsg.concat([
            "The length of expected answer must be at least 3 words. please enter atlest 3 word again.",
          ])
        );
        flag = true;
      }
      if (question.length == 3 && question[2] == -1) {
        setErrorMsg((oldMsg) =>
          oldMsg.concat(["Every MCQ must have a selected answer.please enter atlest 3 word again"])
        );
        flag = true;
      }
      if (question[0].split(" ").length < 3) {
        setErrorMsg((oldMsg) =>
          oldMsg.concat(["The length of question must be at least 3 words.please enter atlest 3 word again"])
        );
        flag = true;
      }
      if (!maxMarks[property]) {
        setErrorMsg((oldMsg) =>
          oldMsg.concat(["Maximum Marks field cannot be left empty..please enter atlest 3 word again"])
        );
        flag = true;
  ...
  let inputElements = inputs.map((value) => {
    ...
    return (
      ...
      <div key={value.id}>
        ...
        {/* <button onClick={()=>{this.handleKeyPress()}}>Add options</button> */}
        <div className="max-score mt-2 d-flex justify-content-end">
          <label htmlFor={`${value.id}`}>Maximum Marks:</label>
          <input
            onChange={handleChange}
